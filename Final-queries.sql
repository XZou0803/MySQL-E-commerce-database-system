...
select ProductName, sum(ItemQuantity) from pop group by 1;

select ShipCountry,count(*) as bestsell from Buyers left join Orders on Buyers.BuyerID = Orders.BuyerID group by ShipCountry;


select Buyers.BuyerID,Products.ProductName from Buyers left join Orders on Buyers.BuyerID = Orders.BuyerID
...
select distinct Buyers.BuyerID, count(*) from Buyers inner join Orders on Orders.BuyerID = Buyers.BuyerID group by 1;

select distinct ProductName,count(*) from Products group by ProductName;

create view bigsupplier as 
...
select SupplierID, count(*) as bigs from bigsupplier group by 1;

select * from Supplier inner join Products on Supplier.SupplierID = Products.SupplierID;

select OrderID, datediff(ShipDate,PaymentDate) from Orders;

# Business metrics(others shown on Tableau)
...
create table Stockcheck (ProductID varchar(11), Stocklevel varchar(100));
...
FOR EACH ROW
begin
case when (new.QuantityOnHand <= 100) 
...
DELIMITER $$
insert into Products_1(ProductID, ProductName, SupplierID, Category, UnitPrice, Color, QuantityOnHand)
values('096419-4825', 'Sets', '43636501699', '', '$60.44', 'orange', 50)
;
$$
...
DELIMITER //
create table price_update (ProductID varchar(11), old_price varchar(100), new_price varchar(100),change_date datetime);
//

DELIMITER //
CREATE PROCEDURE `update_price` (IN ID varchar(11), IN newprice varchar(100))
...
AFTER update on Products_1
FOR EACH ROW
BEGIN
     if old.UnitPrice != new.UnitPrice then
    insert into price_update(ProductID, old_price,new_price,change_date)
      values (OLD.ProductID, old.UnitPrice,New.UnitPrice,curdate())
        ;
END if;
END $$
...
Call update_price('096419-4823', "$101");
